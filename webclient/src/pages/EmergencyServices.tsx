...
export function EmergencyServices() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans flex flex-col justify-between">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-2 sm:gap-4">
            <button 
              ...
              className="px-3 py-1.5 rounded-lg border border-slate-300 hover:border-red-600 text-xs font-bold text-slate-900 flex items-center gap-1.5 transition-colors cursor-pointer bg-white shadow-sm"
            >
      ...
      {/* Emergency Helpline Cards Grid - Clean Light Cards */}
          ...
          {emergencyCards.map((card, idx) => (
            ...
            >
              <div>
                ...
                <div className="text-3xl font-black text-red-600 tracking-tight font-mono mb-2">{card.number}</div>
                <p className="text-xs font-medium text-slate-700 leading-relaxed mb-6">{card.desc}</p>
              </div>

              <a 
                href={`tel:${card.number}`}
                className="w-full py-3 px-4 rounded-xl bg-red-600 hover:bg-red-700 text-white text-xs font-black flex items-center justify-center gap-2 transition-colors cursor-pointer shadow-md"
              >
                <PhoneCall size={16} />
      ...
      {/* Emergency Instructions Section - Clean Light Style */}
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {instructions.map((item, idx) => (
              <div key={idx} className="bg-white p-6 rounded-2xl border-2 border-slate-200 shadow-sm space-y-3">
                <div className="w-10 h-10 rounded-full bg-red-100 text-red-700 border border-red-300 font-extrabold flex items-center justify-center text-sm font-mono">
                  0{item.step}
                </div>
